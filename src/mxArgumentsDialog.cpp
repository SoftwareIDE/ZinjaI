...
wxArrayString mxArgumentsDialog::list_for_combo;
...
mxArgumentsDialog::mxArgumentsDialog(wxWindow *parent,wxString def_text) : wxDialog(parent,wxID_ANY,LANG(ARGUMENTS_CAPTION,"Argumentos para la ejecucion"),wxDefaultPosition,wxDefaultSize,wxALWAYS_SHOW_SB | wxALWAYS_SHOW_SB | wxDEFAULT_FRAME_STYLE | wxSUNKEN_BORDER) {
	...
	mySizer->Add(new wxStaticText(this, wxID_ANY, LANG(ARGUMENTS_ARGUMENTS,"Argumentos para la ejecucion:"), wxDefaultPosition, wxDefaultSize, 0), sizers->BA5_Left);
	
	combo = new wxComboBox(this, wxID_ANY,def_text, wxDefaultPosition, wxDefaultSize, list_for_combo);
	comboSizer->Add(combo, sizers->BA5_Exp1);
	comboSizer->Add(new wxButton(this,mxID_ARGS_BUTTON,_T("..."),wxDefaultPosition,wxSize(30,10)), sizers->BA5_Exp0);
	mySizer->Add(comboSizer, sizers->BA5_Exp1);
...
void mxArgumentsDialog::OnYesButton(wxCommandEvent &evt) {
	if (combo->GetValue().Len()) {
		last_arguments=combo->GetValue();
		if (list_for_combo.GetCount()==0 || (list_for_combo.GetCount() && list_for_combo[list_for_combo.GetCount()-1]!=last_arguments))
...
void mxArgumentsDialog::OnArgsReplaceFile(wxCommandEvent &evt) {
	wxString sel = combo->GetStringSelection();
...
void mxArgumentsDialog::OnArgsAddFile(wxCommandEvent &evt) {
	...
	if (wxID_OK==dlg.ShowModal()) {
		long p1,p2;
		combo->GetSelection(&p1,&p2);
		last_dir = dlg.GetPath();
		combo->Replace(p1,p2,dlg.GetPath());
	}
}
...
void mxArgumentsDialog::OnArgsEditText(wxCommandEvent &evt) {
	new mxLongTextEditor(this,LANG(ARGUMENTS_CAPTION,"Argumentos para la ejecucion"),combo);
}

void mxArgumentsDialog::OnArgsEditList(wxCommandEvent &evt) {
	new mxEnumerationEditor(this,LANG(ARGUMENTS_CAPTION,"Argumentos para la ejecucion"),combo,false);
}
