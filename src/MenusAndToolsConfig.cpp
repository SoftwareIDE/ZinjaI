...
void MenusAndToolsConfig::LoadMenuData ( ) {
	...
	menues[mnEDIT].label = LANG(MENUITEM_EDIT,"&Edicion"); {
		...
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_DELETE_LINES, LANG(MENUITEM_EDIT_DELETE_LINES,"&Eliminar Linea(s)")).ShortCut("Shift+Ctrl+L").Description("Elimina la linea actual del cursor nuevamente, o las lineas seleccionadas").Icon("borrarLineas.png").EnableIf(ecSOURCE));
		AddSeparator(mnEDIT);
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_GOTO_FUNCTION, LANG(MENUITEM_EDIT_GOTO_FUNCTION,"&Ir a Funcion/Clase/Metodo...")).ShortCut("Ctrl+Shift+G").Description("Abrir el fuente con la declaracion de una funcion, clase o metodo...").Icon("irAFuncion.png").EnableIf(ecPROJECT_OR_SOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_GOTO_FILE, LANG(MENUITEM_EDIT_GOTO_FILE,"&Ir a Archivo...")).ShortCut("Ctrl+Shift+F").Description("Abrir un archivo en particular buscandolo por parte de su nombre...").Icon("irAArchivo.png").EnableIf(ecPROJECT_OR_SOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_GOTO, LANG(MENUITEM_EDIT_GOTO_LINE,"&Ir a Linea...")).ShortCut("Ctrl+G").Description("Mover el cursor a una linea determinada en el archivo...").Icon("irALinea.png").EnableIf(ecSOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_FIND, LANG(MENUITEM_EDIT_FIND,"&Buscar...")).ShortCut("Ctrl+F").Description("Buscar una cadena en el archivo...").Icon("buscar.png").EnableIf(ecPROJECT_OR_SOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_FIND_PREV, LANG(MENUITEM_EDIT_FIND_PREV,"Buscar &Anterior")).ShortCut("Shift+F3").Description("Repetir la ultima busqueda a partir del cursor hacia atras").Icon("buscarAnterior.png").EnableIf(ecSOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_FIND_NEXT, LANG(MENUITEM_EDIT_FIND_NEXT,"Buscar &Siguiente")).ShortCut("F3").Description("Repetir la ultima busqueda a partir del cursor").Icon("buscarSiguiente.png").EnableIf(ecSOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_REPLACE, LANG(MENUITEM_EDIT_REPLACE,"&Reemplazar...")).ShortCut("Ctrl+R").Description("Reemplazar una cadena con otra en el archivo...").Icon("reemplazar.png").EnableIf(ecSOURCE));
		AddSeparator(mnEDIT);
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_INSERT_HEADER, LANG(MENUITEM_EDIT_INSERT_INCLUDE,"Insertar #include Correspondiente")).ShortCut("Ctrl+H").Description("Si es posible, inserta el #include necesario para utilizar la funcion/clase en la cual se encuentra el cursor.").Icon("insertarInclude.png").EnableIf(ecSOURCE));
		AddMenuItem(mnEDIT, myMenuItem(mxID_EDIT_COMMENT, LANG(MENUITEM_EDIT_COMMENT,"Comentar")).ShortCut("Ctrl+D").Description("Convierte el texto seleccionado en comentario anadiendo \"//\" a cada linea").Icon("comentar.png").EnableIf(ecSOURCE));
	...
	menues[mnVIEW].label = LANG(MENUITEM_VIEW,_if_not_apple("&Ver","Ver")); {
		AddMenuItem(mnVIEW, myMenuItem(mxID_VIEW_DUPLICATE_TAB, LANG(MENUITEM_VIEW_SPLIT_VIEW,"&Duplicar vista")).Icon("duplicarVista.png").EnableIf(ecSOURCE));
		...
		EndSubMenu(mnVIEW);
		AddSeparator(mnVIEW);
		AddMenuItem(mnVIEW, myMenuItem(mxID_VIEW_PREV_ERROR, LANG(MENUITEM_VIEW_PREV_ERROR,"&Ir a error anterior")).ShortCut(_if_win32("Ctrl+>","Ctrl+Shift+<")).Description("Selecciona el error/advertencia anterior de la salida del compilador.").Icon("errorPrev.png").EnableIf(ecSOURCE));
		AddMenuItem(mnVIEW, myMenuItem(mxID_VIEW_NEXT_ERROR, LANG(MENUITEM_VIEW_NEXT_ERROR,"&Ir a siguiente error")).ShortCut("Ctrl+<").Description("Selecciona el proximo error/advertencia de la salida del compilador.").Icon("errorNext.png").EnableIf(ecSOURCE));
	}
	
	
	menues[mnRUN].label = LANG(MENUITEM_RUN,"E&jecucion"); {
		AddMenuItem(mnRUN, myMenuItem( mxID_RUN_RUN, LANG(MENUITEM_RUN_RUN,"&Ejecutar...")).ShortCut("F9").Description("Guarda y compila si es necesario, luego ejecuta el programa").Icon("ejecutar.png").Map().EnableIf(ecPROJECT_OR_SOURCE));
		AddMenuItem(mnRUN, myMenuItem( mxID_RUN_RUN_OLD, LANG(MENUITEM_RUN_OLD,"Ejecutar (sin recompilar)...")).ShortCut("Ctrl+F9").Description("Ejecuta el binario existente sin recompilar primero").Icon("ejecutar_old.png").EnableIf(ecPROJECT_OR_SOURCE));
		...
		AddMenuItem(mnRUN, myMenuItem( mxID_RUN_CONFIG, LANG(MENUITEM_RUN_OPTIONS,"&Opciones...")).ShortCut("Alt+F9").Description("Configura la compilacion y ejecucion de los programas").Icon("opciones.png").EnableIf(ecPROJECT_OR_SOURCE));
	}
	...
	menues[mnDEBUG].label = LANG(MENUITEM_DEBUG,"&Depuracion"); {
		...
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_PAUSE, LANG(MENUITEM_DEBUG_PAUSE,"Interrum&pir")).Icon("pausar.png").EnableIf(ecDEBUG_NOT_PAUSED));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_STOP, LANG(MENUITEM_DEBUG_STOP,"&Detener")).ShortCut("Shift+F5").Icon("detener.png").EnableIf(ecDEBUG));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_STEP_IN, LANG(MENUITEM_DEBUG_STEP_IN,"Step &In")).ShortCut("F6").Icon("step_in.png").EnableIf(ecDEBUG_PAUSED));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_STEP_OVER, LANG(MENUITEM_DEBUG_STEP_OVER,"Step &Over")).ShortCut("F7").Icon("step_over.png").EnableIf(ecDEBUG_PAUSED));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_STEP_OUT, LANG(MENUITEM_DEBUG_STEP_OUT,"Step O&ut")).ShortCut("Shift+F6").Icon("step_out.png").EnableIf(ecDEBUG_PAUSED));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_RUN_UNTIL, LANG(MENUITEM_DEBUG_RUN_UNTIL,"Ejecutar &Hasta el Cursor")).ShortCut("Shift+F7").Icon("run_until.png").EnableIf(ecDEBUG_PAUSED));
		AddMenuItem(mnDEBUG, myMenuItem( mxID_DEBUG_RETURN, LANG(MENUITEM_DEBUG_RETURN,"&Return")).ShortCut("Ctrl+F6").Icon("return.png").EnableIf(ecDEBUG_PAUSED));
		...
		BeginSubMenu(mnDEBUG, LANG(MENUITEM_DEBUG_MORE,"Más..."));
			AddMenuItem(mnDEBUG, myMenuItem(mxID_DEBUG_ATTACH, LANG(MENUITEM_DEBUG_ATTACH,"&Adjuntar...")).Icon("debug_attach.png").EnableIf(ecNOT_DEBUG));
			AddMenuItem(mnDEBUG, myMenuItem(mxID_DEBUG_LOAD_CORE_DUMP, LANG(MENUITEM_DEBUG_LOAD_CORE_DUMP,"Cargar &Volcado de Memoria...")).Icon("core_dump.png").EnableIf(ecNOT_DEBUG));
...
#ifndef __WIN32__
			AddMenuItem(mnDEBUG, myMenuItem(mxID_DEBUG_PATCH, LANG(MENUITEM_DEBUG_PATCH,"Actualizar ejecutable (experimental)...")).Icon("debug_patch.png").EnableIf(ecDEBUG_PAUSED));
		EndSubMenu(mnDEBUG);
#endif
	}
	...
	menues[mnTOOLS].label = LANG(MENUITEM_TOOLS,"&Herramientas"); {
		AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_DRAW_FLOW, LANG(MENUITEM_TOOLS_DRAW_FLOWCHART,"Dibujar Diagrama de &Flujo...")).Description("Genera un diagrama de flujo a partir del bloque de codigo actual").Icon("flujo.png").EnableIf(ecSOURCE));
		AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_DRAW_CLASSES, LANG(MENUITEM_TOOLS_DRAW_CLASS_HIERARCHY,"Dibujar &Jerarquia de Clases...")).Icon("clases.png").EnableIf(ecPROJECT_OR_SOURCE));
		
		AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_CODE_COPY_FROM_H, LANG(MENUITEM_TOOLS_CODE_COPY_FROM_H,"Implementar Métodos/Funciones faltantes...")).ShortCut("Ctrl+Shift+H").Icon("copy_code_from_h.png").EnableIf(ecSOURCE));
		...
		AddMenuItem(mnTOOLS, myMenuItem( mxID_TOOLS_MAKEFILE, LANG(MENUITEM_TOOLS_GENERATE_MAKEFILE,"&Generar Makefile...")).Description("Genera el Makefile a partir de los fuentes y la configuracion seleccionada").Icon("makefile.png").EnableIf(ecPROJECT));
		...
		AddMenuItem(mnTOOLS, myMenuItem( mxID_TOOLS_EXE_PROPS, LANG(MENUITEM_TOOLS_EXE_INFO,"&Propiedades del Ejecutable...")).Description("Muestra informacion sobre el archivo compilado").Icon("exeinfo.png").EnableIf(ecPROJECT_OR_SOURCE));
		AddMenuItem(mnTOOLS, myMenuItem( mxID_TOOLS_PROJECT_STATISTICS, LANG(MENUITEM_TOOLS_PROJECT_STATISTICS,"E&stadisticas del Proyecto...")).Description("Muestra informacion estadistica sobre los fuentes y demas archivos del proyecto").Icon("proystats.png").EnableIf(ecPROJECT));
		AddMenuItem(mnTOOLS, myMenuItem( mxID_TOOLS_DRAW_PROJECT, LANG(MENUITEM_TOOLS_DRAW_PROJECT,"Grafo del Proyecto...")).Icon("draw_project.png").EnableIf(ecPROJECT));
		...
		BeginSubMenu(mnTOOLS,LANG(MENUITEM_TOOLS_SHARE,"Compartir Archivos en la &Red Local"),"Permite enviar o recibir codigos fuentes a traves de una red LAN","share.png");
			...
			AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_SHARE_SHARE, LANG(MENUITEM_TOOLS_SHARE_SHARE,"&Compartir actual...")).Description("Comparte el archivo en la red local.").Icon("compartir.png").EnableIf(ecSOURCE));
		...
		BeginSubMenu(mnTOOLS,myMenuItem(wxID_ANY,LANG(MENUITEM_TOOLS_DOXYGEN,"Generar &Documentación (doxygen)")).Description("Doxygen permite generar automaticamente documentacion a partir del codigo y sus comentarios").Icon("doxy.png").EnableIf(ecPROJECT));
		...
		BeginSubMenu(mnTOOLS,myMenuItem(wxID_ANY,LANG(MENUITEM_TOOLS_GPROF,"Perfil de Ejecución (gprof)")).Description("Gprof permite analizar las llamadas a funciones y sus tiempos de ejecucion.").Icon("gprof.png").EnableIf(ecPROJECT_OR_SOURCE));
		...
		BeginSubMenu(mnTOOLS, LANG(MENUITEM_TOOLS_CUSTOM_TOOLS,"Herramientas Personalizables"),"","customTools.png",mxID_TOOLS_CUSTOM_TOOLS,maMAPPED);
			...
			AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_PROJECT_TOOLS_SETTINGS, LANG(MENUITEM_TOOLS_PROJECT_TOOLS_SETTINGS,"&Configurar (de proyecto)...")).Icon("projectToolsSettings.png").EnableIf(ecPROJECT));
			AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_CUSTOM_HELP, LANG(MENUITEM_TOOLS_CUSTOM_HELP,"A&yuda...")).Icon("ayuda.png"));
		EndSubMenu(mnTOOLS);
		
		AddSeparator(mnTOOLS);
		
		AddMenuItem(mnTOOLS, myMenuItem(mxID_TOOLS_CREATE_TEMPLATE, LANG(MENUITEM_TOOLS_CREATE_TEMPLATE,"Guardar como nueva plantilla...")).Description("Permite guardar el programa simple o proyecto actual como plantilla").Icon("create_template.png").EnableIf(ecPROJECT_OR_SOURCE));
